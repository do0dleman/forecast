...
export default function WeatherCard(props: WeatherCardProps) {
    const { className, activeItem, activeItem2, title, subTitle, icon, ...rest } = props
    const [time, setTime] = useState(new Date())

    return (
        <Box className={`${className} weather-card`} {...rest}>
            <div className="weather-card__side">
                ...
                {activeItem2 ?
                    <ActiveItem2 className='weather-card__item' item={activeItem} item2={activeItem2} /> :
                    <ActiveItem className='weather-card__item' item={activeItem} />}
            </div>
